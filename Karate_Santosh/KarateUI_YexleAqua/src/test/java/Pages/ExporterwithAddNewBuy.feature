Feature: Add New Buy Button with functionality
	...
	* delay(5000)
	And click(addDetailsLink)
	* delay(5000)
	Then input(firstName, 'sandy')
	* delay(2000)	
	Then input(lastName, 'kumar')
	* delay(2000)
	Then input(role, 'Tester')
	* delay(2000)
	Then input(phone, '9876543210')
	* delay(2000)
	Then input(email, 'sandy@yexle.com')
	* delay(2000)
	Then input(address, 'Hyderabad')
	* delay(2000)
	Then input(comment, 'This is for demo purpose we are sending data to the comment field')
	* delay(2000)	
	* scroll(nextButton1)
	* highlight(nextButton1)	
